    ...
    if not check_user(user):
        ...
        try:
            ...
            if completed:
                ...
                task=Task.objects.filter(**query_filter).order_by("-priority").exclude(status=Status.objects.get(name="Completado"))
                return task
            task=Task.objects.filter(**query_filter).order_by("-priority")
            return task
